//! Tray + main-window lifecycle helpers used by `run()`.
use std::sync::Arc;
...
use crate::domain::LocaleCode;
use crate::{build_tray_menu_public, tray_copy_public, EVENT_NAVIGATE};
...
pub fn apply_launch_minimized(app: &tauri::App) {
...
pub fn wire_close_to_hide(app: &tauri::App) {
...
pub fn wire_failure_hook(app: &tauri::App) {
    let app_handle = app.handle().clone();
    let service_hook = Arc::clone(&app.state::<AppState>().service);
    crate::infra::set_failure_hook(move |name| {
