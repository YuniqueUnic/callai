...
## 附录 · Actions 审批：fork 首次贡献者 vs bot PR 假象
...
| `approval_policy` | 含义 |
| --- | --- |
| `first_time_contributors_new_to_github` | 仅 GitHub 全新账号的首次 fork PR 需批 |
| **`first_time_contributors`** | **对本仓首次贡献的外部作者** fork PR 需批（推荐） |
| `all_external_contributors` | 所有外部作者每次都需批（过严） |
...
### 常见误会：`action_required` 不全是「首次贡献者」

`packaging-sync` 用 `GITHUB_TOKEN` push 分支并 `gh pr create` 时，GitHub **默认不给这个 PR 跑 `pull_request` workflows**（安全限制，防递归 token 滥用）。  
UI 上也会显示 `action_required` / no checks，**看起来像要手动 approve gate**，但：

- 不是 fork first-time policy（同仓 bot PR，不是 fork）
- `gh api .../runs/{id}/approve` 会 403：`This run is not from a fork pull request`

**修法（callai）**：

1. `CI` 增加 `workflow_dispatch`
2. `packaging-sync` 开 PR 后：`gh workflow run CI --ref $BRANCH`（及 Packaging）
3. 维护者也可本地 empty commit 以用户身份重推触发

### 默认 workflow token 权限
...
`write` 便于 release/packaging 写权限；仍靠 secrets + branch protection 守门。
